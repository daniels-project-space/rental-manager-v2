...
export const triggerWorkflow = internalAction({
  ...
  handler: async (ctx) => {
    ...
    // Both downstream routes (Mastra `/api/workflows/hygglo-poll` and the
    // redundant L3 backup `/api/poll-hygglo`) authenticate using the same
    // `POLL_TRIGGER_SECRET` env var, sent as `Authorization: Bearer <secret>`.
    try {
