...
export default defineSchema({
  ...
  // ── Bulk listing-price change: proposals + audit (2026-08-14) ─────────────
  // Backs convex/listing_price_admin.ts. The price-write path edits LIVE Hygglo
  // listings (real money), so it is deliberately two-phase: a read-only dry run
  // freezes a proposal here, and the operator/agent must hand the returned
  // `token` back to `executePriceChange`. The action ALSO requires
  // ALLOW_LISTING_PRICE_WRITES==="true" — the token is the "which change" proof,
  // the env var is the "writes are switched on at all" gate. Neither implies
  // the other.
  //
  // A proposal is single-use: `consumed_at` is stamped BEFORE any Hygglo call so
  // a retried/duplicated invocation cannot double-apply a percentage increase.
  price_change_proposals: defineTable({
    token: v.string(),                       // opaque, generated by createPriceChangeProposal
    account_slug: v.string(),
    percent_change: v.number(),              // e.g. +10 = *1.10, -5 = *0.95
    // Frozen dry-run diff. Advisory only: executePriceChange RE-COMPUTES from
    // live cache and never trusts these numbers for the actual write. Kept so
    // the operator can see what they approved vs what was applied.
    proposed: v.array(
      v.object({
        listing_id: v.number(),
        name: v.optional(v.string()),
    ...
    created_at: v.number(),
    expires_at: v.number(),                  // created_at + PROPOSAL_TTL_MS
    consumed_at: v.optional(v.number()),     // set once; a consumed token is dead
    source: v.optional(v.string()),          // "walle" | "manual" | …
  })
    .index("by_token", ["token"])
    .index("by_account", ["account_slug"]),

  // One row per listing per execution attempt — success AND failure. Written
  // even when the Hygglo write throws, so a partially-applied batch is always
  // reconstructable after the fact.
  price_change_audit: defineTable({
    account_slug: v.string(),
    listing_id: v.number(),
    old_price: v.number(),
    new_price: v.number(),
    changed_at: v.number(),                  // Date.now() at write time
    source: v.string(),                      // "walle" | "manual" | …
    success: v.boolean(),
    error: v.optional(v.string()),
  })
    .index("by_account", ["account_slug"])
