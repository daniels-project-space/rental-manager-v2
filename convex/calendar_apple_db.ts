...
export const _getReservationForCal = internalQuery({
  ...
  handler: async (ctx, { thread_id }): Promise<ResForCal | null> => {
    ...
    return {
      thread_id, account_slug: (a.account_slug as string) ?? null,
      renter_name: (a.renter_name as string) ?? "renter",
      items: ((a.hygglo_items as Array<{ name: string }> | undefined) ?? []).map((h) => h.name),
      status: (a.status as string) ?? "",
      pickup_date: (a.pickup_date as string) ?? (a.start_date as string) ?? null,
      pickup_time: (a.pickup_time as string) ?? null,
      return_date: (a.return_date as string) ?? (a.end_date as string) ?? null,
