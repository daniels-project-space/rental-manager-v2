 ...
 *
 * Phase 18.3 refactor: pure compute consumes pre-collected reservations
 * + items. `refresh` internalMutation stays as thin wrapper.
 *
 * Tracks per-item utilisation snapshot:
...
export type UtilizationItemRow = {
  itemName: string;
  capacity: number;
  rentedNow: number;
  idleDays7d: number;
...
export type UtilizationAccountRow = {
  account: string;
  generatedAt: number;
  rows: UtilizationItemRow[];
  fleetUtilizationPct: number;
};

/**
 * Pure compute — reservations should be the 30-day window slice.
 * Items is the full items table.
 */
export function computeUtilization(args: {
  reservations: ReservationRow[];
  items: ItemRow[];
  targetAccount?: string;
  generatedAt?: number;
}): UtilizationAccountRow[] {
  ...
  const result: UtilizationAccountRow[] = [];
  ...
  for (const account of targets) {
    ...
    const rows: UtilizationItemRow[] = activeItems.map((it) => {
    ...
    result.push({ account, generatedAt, rows, fleetUtilizationPct });
  }
