...
import {
  ...
  MISSED_PALETTE,
  MISSED_OTHER_COLOR,
  MISSED_UNMATCHED_COLOR,
} from "./mv/missed_denied_compute";

// `isOwnerDeniedLike` moved to ./mv/missed_denied_compute (imported above) so the
// single-pass compute and revenue.ts drill-downs share ONE definition.
...
// r2, MISSED_KIND_LABELS, MISSED_PALETTE, MISSED_OTHER_COLOR,
// MISSED_UNMATCHED_COLOR, missedLabelFor, kindFromKeywords moved to
// ./mv/missed_denied_compute (imported above). MISSED_KIND_LABELS is re-imported
// only for any downstream reference; void it here to keep the symbol referenced
// without changing behavior.
void MISSED_KIND_LABELS;
 ...
 *
 * Single-pass refactor (2026-06-02): the live branch now delegates to the pure
 * `computeMissedAndDeniedByCategory` over one `fetchMissedDeniedData` snapshot.
 * Output is byte-identical to the prior full-scan implementation (parity-verified
 * 2026-06-02 across all 9 account×window cells before deploy).
 */
export const getMissedAndDeniedByCategory = query({
  ...
  handler: async (ctx, { accountSlug, days, _bypassMv }) => {
    ...
    // Single-pass refactor (2026-06-02): the 4 reads happen ONCE, then the pure
    // compute derives this cell in memory. Byte-identical to the prior full-scan
    // implementation (parity-verified before deploy).
    const now = Date.now();
    const data = await fetchMissedDeniedData(ctx, now);
    return computeMissedAndDeniedByCategory(data, { accountSlug, days, now });
  },
});
